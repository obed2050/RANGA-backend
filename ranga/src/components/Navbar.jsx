...
import { FiShoppingCart, FiSearch, FiMenu, FiX, FiLogOut, FiShield } from "react-icons/fi";
      ...
      <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-4">
        ...
        <form onSubmit={handleSearch} className="flex-1 flex">
          <input
            ...
            className="w-full border-2 border-gray-300 rounded-l-full px-4 py-2 text-sm focus:outline-none focus:border-yellow-500"
          />
          <button
            type="submit"
        ...
        <div className="hidden md:flex items-center gap-4">
          {isAuthenticated ? (
            <>
              ...
              <button onClick={handleLogout} className="flex items-center gap-1 text-sm text-gray-700 hover:text-yellow-600 transition">
                <FiLogOut /> Logout
              </button>
            </>
          ) : (
            <>
              <Link to="/login" className="text-sm font-medium text-gray-700 hover:text-yellow-600 transition">Login</Link>
          ...
          <Link to="/cart" className="relative">
            <FiShoppingCart size={24} className="text-gray-700 hover:text-yellow-600 transition" />
            {totalItems > 0 && (
        ...
        <div className="md:hidden bg-white border-t px-4 py-3 flex flex-col gap-3">
          {isAuthenticated ? (
            <>
              ...
              {user?.role !== "admin" && (
                <Link to="/orders" onClick={() => setMenuOpen(false)} className="text-sm text-gray-700">Orders</Link>
              )}
