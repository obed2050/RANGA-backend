          ...
          <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm overflow-hidden">
            <div className="grid md:grid-cols-2 gap-0">
              ...
              <div className="p-6 flex flex-col gap-4">
                {product.category && (
                  <span className="text-xs font-semibold text-yellow-700 bg-yellow-50 px-3 py-1 rounded-full w-fit">
                    {product.category}
                  </span>
                )}
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center border border-gray-300 rounded-xl overflow-hidden">
                    <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="px-3 py-2 hover:bg-gray-100 transition">
                      <FiMinus size={13} />
                    </button>
                    <span className="px-4 py-2 font-semibold text-gray-800 border-x border-gray-300">{quantity}</span>
                    <button onClick={() => setQuantity(quantity + 1)} className="px-3 py-2 hover:bg-gray-100 transition">
                      <FiPlus size={13} />
                    </button>
                  </div>
                </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-sm p-6 sticky top-24">
              ...
              <div className="space-y-3 mb-5">
                {/* Telefoni — ibitswe niba utakoze login */}
                <div className="flex items-start gap-3 p-3 bg-gray-50 rounded-xl">
                  <FiPhone className="text-yellow-500 mt-0.5 flex-shrink-0" size={15} />
                  <div className="flex-1">
                    <p className="text-xs text-gray-500 mb-0.5">Telefoni</p>
                    {isAuthenticated ? (
                      <p className="text-sm font-semibold text-gray-800">{sellerPhone}</p>
                    ) : (
                ...
                <div className="flex items-start gap-3 p-3 bg-gray-50 rounded-xl">
                  ...
                  <div>
                    <p className="text-xs text-gray-500 mb-0.5">Aderesi</p>
                    <p className="text-sm font-semibold text-gray-800">{sellerAddress}</p>
                  </div>
                </div>
                ...
                {sellerDesc && (
                  <div className="p-3 bg-gray-50 rounded-xl">
                    ...
                    <p className="text-xs text-gray-700 leading-relaxed">{sellerDesc}</p>
                  </div>
                )}
              </div>

              <div className="bg-yellow-50 rounded-xl p-3 mb-4 text-center">
                <p className="text-xs text-gray-500">Igiciro cy'ibyo ushaka</p>
              ...
              {/* Buttons — protected niba utakoze login */}
              {isAuthenticated ? (
                <div className="flex flex-col gap-3">
                  <a
                    href={`https://wa.me/${sellerWhatsapp}?text=${message}`}
                    target="_blank"
