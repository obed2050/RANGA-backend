...
export default function AdminDashboard() {
  ...
  const fetchAll = async () => {
    ...
    try {
      const [uRes, pRes, sRes] = await Promise.all([
        adminGetUsers(), adminGetProducts(), adminGetShops(),
      ]);
      setUsers(uRes.data || []);
  ...
  const stats = [
    { label: "Abakoresha Bose", value: users.length, icon: FiUsers, color: "bg-blue-50 text-blue-600", sub: `${sellers.length} sellers, ${buyers.length} buyers` },
    { label: "Ibicuruzwa", value: products.length, icon: FiPackage, color: "bg-yellow-50 text-yellow-600", sub: "Total products" },
    { label: "Amasoko", value: shops.length, icon: FiShoppingBag, color: "bg-purple-50 text-purple-600", sub: "Active shops" },
    { label: "Total Value", value: `$${products.reduce((s, p) => s + Number(p.price || 0), 0).toFixed(0)}`, icon: FiTrendingUp, color: "bg-green-50 text-green-600", sub: "Products value" },
  ];
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex gap-2 mb-6 bg-gray-100 p-1 rounded-xl w-fit">
        {TABS.map((tab) => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            className={`px-5 py-2 rounded-lg text-sm font-medium transition ${activeTab === tab ? "bg-white shadow text-yellow-600" : "text-gray-500 hover:text-gray-700"}`}>
            {tab}
          </button>
        ))}
      </div>
      ...
      {activeTab === "Overview" && (
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-white rounded-2xl shadow-sm p-5">
            <h3 className="font-bold text-gray-800 mb-4">Sellers Bashya</h3>
            <div className="space-y-3">
              {sellers.slice(0, 5).map((s) => (
                <div key={s.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                  <div className="w-9 h-9 bg-yellow-100 rounded-full flex items-center justify-center text-yellow-600 font-bold text-sm flex-shrink-0">{s.name.charAt(0)}</div>
          ...
          <div className="bg-white rounded-2xl shadow-sm p-5">
            <h3 className="font-bold text-gray-800 mb-4">Ibicuruzwa Bishya</h3>
            <div className="space-y-3">
              {products.slice(0, 5).map((p) => (
                <div key={p.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                  <div className="w-9 h-9 bg-yellow-50 rounded-lg flex items-center justify-center"><FiPackage size={16} className="text-yellow-500" /></div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-800 truncate">{p.name}</p>
                    <p className="text-xs text-gray-500">by {p.shop?.name || "—"}</p>
                  </div>
      ...
      {activeTab === "Abakoresha" && (
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          <div className="p-5 border-b border-gray-100 flex items-center justify-between">
            <h2 className="font-bold text-gray-800">Abakoresha Bose ({filteredUsers.length})</h2>
            <div className="flex gap-2">
              {["all", "seller", "buyer", "admin"].map((r) => (
                <button key={r} onClick={() => setFilterRole(r)}
                  className={`text-xs px-3 py-1.5 rounded-lg font-medium transition capitalize ${filterRole === r ? "bg-yellow-500 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
                  {r === "all" ? "Bose" : r}
                </button>
              ))}
            </div>
                  ...
                  <tr key={u.id} className="hover:bg-gray-50 transition">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 bg-yellow-100 rounded-full flex items-center justify-center text-yellow-600 font-bold text-sm flex-shrink-0">{u.name.charAt(0)}</div>
                        <div><p className="font-medium text-gray-800">{u.name}</p><p className="text-xs text-gray-400">{u.email}</p></div>
                      </div>
                    </td>
                    <td className="px-5 py-4">
                      <span className={`text-xs px-2 py-1 rounded-full font-medium ${u.role === "seller" ? "bg-yellow-50 text-yellow-700" : u.role === "admin" ? "bg-red-50 text-red-700" : "bg-blue-50 text-blue-700"}`}>{u.role}</span>
                    </td>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex gap-2">
                        {u.phone && <a href={`https://wa.me/${u.phone.replace(/^0/, "250")}`} target="_blank" rel="noopener noreferrer" className="p-2 text-green-500 hover:bg-green-50 rounded-lg transition"><FaWhatsapp size={14} /></a>}
                        {u.id !== user?.id && (
                          <button onClick={() => delUser(u.id)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition"><FiTrash2 size={14} /></button>
                        )}
                      </div>
                    </td>
                  </tr>
      ...
      {activeTab === "Ibicuruzwa" && (
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          <div className="p-5 border-b border-gray-100">
            <h2 className="font-bold text-gray-800">Ibicuruzwa Byose ({products.length})</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-gray-500 text-xs uppercase">
              ...
              <tbody className="divide-y divide-gray-50">
                {products.map((p) => (
                  <tr key={p.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-5 py-4"><span className="bg-yellow-50 text-yellow-700 text-xs px-2 py-1 rounded-full">{p.category?.name || "—"}</span></td>
                    <td className="px-5 py-4 text-gray-600">{p.shop?.name || "—"}</td>
                    ...
                    <td className="px-5 py-4">
                      <button onClick={() => delProduct(p.id)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition"><FiTrash2 size={14} /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {activeTab === "Amasoko" && (
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
          <div className="p-5 border-b border-gray-100">
            <h2 className="font-bold text-gray-800">Amasoko Yose ({shops.length})</h2>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-gray-50 text-gray-500 text-xs uppercase">
              ...
              <tbody className="divide-y divide-gray-50">
                {shops.map((s) => (
                  <tr key={s.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-5 py-4"><span className={`text-xs px-2 py-1 rounded-full ${s.isActive ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}>{s.isActive ? "Active" : "Inactive"}</span></td>
                    <td className="px-5 py-4">
                      <button onClick={() => delShop(s.id)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition"><FiTrash2 size={14} /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
