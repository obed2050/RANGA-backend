      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          {cartItems.map((item) => {
            ...
            return (
            <div key={iid} className="bg-white rounded-xl shadow-sm p-4 flex gap-4 items-center">
              <img
              ...
              <div className="flex-1 min-w-0">
                <Link to={`/product/${iid}`} className="font-semibold text-gray-800 hover:text-yellow-600 transition text-sm line-clamp-2">
                  {title}
                </Link>
              ...
              <div className="flex flex-col items-end gap-3">
                ...
                <div className="flex items-center border border-gray-200 rounded-lg overflow-hidden">
                  <button onClick={() => updateQuantity(iid, item.quantity - 1)} className="px-2 py-1 hover:bg-gray-100 transition">
                    <FiMinus size={12} />
                  </button>
                ...
                <p className="text-sm font-bold text-gray-700">${(Number(item.price) * item.quantity).toFixed(2)}</p>
              </div>
            </div>
            );
          })}
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
            ...
            <button
              ...
              className="w-full bg-yellow-500 text-white py-3 rounded-xl font-semibold hover:bg-yellow-600 transition mt-4"
            >
              Proceed to Checkout
            </button>
            <button
              onClick={clearCart}
              className="w-full text-gray-500 text-sm mt-2 hover:text-red-500 transition"
            >
              Clear Cart
            </button>
          </div>
